...
//
// Injects canned rows so unit tests never touch a real Postgres. Lives in the
// same package so it can satisfy the unexported querier/rowsIterator interfaces.
// grep pgxpool in _test.go -> nothing (real DB never reached).

// fakeRows is a rowsIterator over a fixed slice of value-rows.
type fakeRows struct {
	cols []string
	data [][]any // one inner slice per row, parallel to cols
	i    int
...
// withQuerier is the test-only injection point for the seam.
func withQuerier(q querier) func(*Source) {
	return func(s *Source) { s.q = q }
}

// newTestSource wires a Source with config already parsed + a fake querier.
// Skips the real DSN/connect path (no pgxpool, no Ping): sets s.rows directly
...
func TestSource_Init_ParsesValidConfig(t *testing.T) {
	...
	// up to the point of connection construction. Capture that by checking the
...
func TestSource_Init_InvalidJSON_WrappedError(t *testing.T) {
	...
	// Must be wrapped with the "postgres source: config:" prefix.
	if !contains(err.Error(), "postgres source: config:") {
...
func TestSource_Init_MissingRequired(t *testing.T) {
	cases := []struct {
		...
		{"host", `{"database":"d","user":"u","password":"p","table":"t","columns":"id"}`, "host"},
		{"database", `{"host":"h","user":"u","password":"p","table":"t","columns":"id"}`, "database"},
		{"user", `{"host":"h","database":"d","password":"p","table":"t","columns":"id"}`, "user"},
		{"password", `{"host":"h","database":"d","user":"u","table":"t","columns":"id"}`, "password"},
		{"table", `{"host":"h","database":"d","user":"u","password":"p","columns":"id"}`, "table"},
		{"columns", `{"host":"h","database":"d","user":"u","password":"p","table":"public.users","columns":""}`, "columns"},
	}
	for _, c := range cases {
